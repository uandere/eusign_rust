...
#include "EUSignUsageConstants.h"
...
std::string readFileToString(const std::string &filePath)
{
	...
	if (!file)
	{
		std::cerr << "IIT EU Sign Usage: �� ������� ������� ����: " << filePath << std::endl;
		return "";
	}
...
CASettings GetCA(std::string caSubjectCN)
{
	CASettings ca;
	for (int i = 0; i < CAs.size(); i++)
	{
		...
		{
			if (caSubjectCN == CAs[i].issuerCNsv[j])
				return CAs[i];
		}
	}
	return ca;
...
BankIDResponse parseBID(const std::string &json)
{
	BankIDResponse resp;

	std::string state = getValue(json, "state");
	if (!state.empty())
	{
		state.erase(0, state.find_first_of("\"") + 1);
		state.erase(state.find_last_of("\""));
		resp.state = state;
	}

	std::string cert = getValue(json, "cert");
	if (!cert.empty())
	{
		cert.erase(0, cert.find_first_of("\"") + 1);
		cert.erase(cert.find_last_of("\""));
		resp.cert = cert;
	}

	std::string customerCrypto = getValue(json, "customerCrypto");
	if (!customerCrypto.empty())
	{
		customerCrypto.erase(0, customerCrypto.find_first_of("\"") + 1);
		customerCrypto.erase(customerCrypto.find_last_of("\""));
		resp.customerCrypto = customerCrypto;
	}
...
BankIDResponse parseBankIDResponse(const std::string &jsonArray)
{
	BankIDResponse resp;
	...
	std::string data)
{
	...
	if (!outFile)
	{
		std::cerr << "IIT EU Sign Usage: �� ������� ������� ���� " << filePath << " ��� ������\n";
	}
...
/*char		g_szSenderCert[] =
	"";

char		g_szCustomerCrypto[] =
	"";
*/
...
char		g_szCustomerCrypto[] = "";
...
int main()
{
	...
	customerData.erase(customerData.size() - 1, 1);
	customerData.erase(0, 1);

	pszTmp = CP1251ToUTF8(pszCustomerData);
	WriteAllText("./Modules/Data/decryptedData.json", pszTmp);
	ReleaseUTF8String(pszTmp);


	pszTmp = CP1251ToUTF8("IIT EU Sign Usage: The response was decoded successfully: ");
	PrintMessage(pszTmp);
	ReleaseUTF8String(pszTmp);
	std::cout << "\t";
	std::cout << customerData << "\n";
	pszTmp = CP1251ToUTF8("IIT EU Sign Usage: Information about sender: ");
	PrintMessage(pszTmp);
	ReleaseUTF8String(pszTmp);
	pszTmp = CP1251ToUTF8("\tSender: ");
	PrintMessage(pszTmp);
	ReleaseUTF8String(pszTmp);
	std::cout << "\t\t";
	...
	ReleaseUTF8String(pszTmp);
	std::cout << "\t";
	pszTmp = CP1251ToUTF8("Key Certification Center which issued the key: ");
	PrintMessage(pszTmp);
	ReleaseUTF8String(pszTmp);
	std::cout << "\t\t";
	...
	ReleaseUTF8String(pszTmp);
	std::cout << "\t";
	pszTmp = CP1251ToUTF8("Certificate Serial Number: ");
	PrintMessage(pszTmp);
	ReleaseUTF8String(pszTmp);
	std::cout << "\t\t";
	...
	PrintMessage(pszTmp);
	ReleaseUTF8String(pszTmp);
	pszTmp = CP1251ToUTF8("IIT EU Sign Usage: Information about signature: ");
	PrintMessage(pszTmp);
	ReleaseUTF8String(pszTmp);
	std::cout << "\t";
	pszTmp = CP1251ToUTF8("Signer: ");
	PrintMessage(pszTmp);
	ReleaseUTF8String(pszTmp);
	std::cout << "\t\t";
